...
class MainCoroutineExitShouldHavePriorityOverHandlerWorkflow:
    ...
    async def this_signal_always_executes_second(self):
        ...
        raise ApplicationError(
            "I don't expect this to be raised if signal 1 CAN'd",
        )
...
class SecretDetectorWorkflow:
    ...
    def check_data(self):
        if self.data and self.data.startswith("<SECRET>"):
            raise ApplicationError("secret detected, shutting down immediately")

    @workflow.update
    async def read_data(self) -> str:
        await workflow.wait_condition(lambda: self.data is not None)
        return self.data
